...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // declares the variable to store the matching results
  std::smatch smatch;
  // declares the regex of what character sequence should be searched for
  std::regex regex("([A-Za-z0-9]'='[A-Za-z0-9]|([0-9]=[0-9]))");

  // if the character sequence is found within the users input 
  if (std::regex_search(sql.begin(), sql.end(), smatch, regex)) {
      // a warning will be displayed to the console
